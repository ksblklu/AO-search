...
typedef BOOL (CALLBACK* CompareFunc)( LPARAM lParam1, LPARAM lParam2);
...
#pragma pack(pop)
	...
	//search for an item after posParent, based on the compare function, posParent = NULL instead of after root
	POSITION FindAfterPos( /*[in]*/ const _ITEMDATA *pItemData, /*[in]*/ POSITION posParent=NULL);
...
{
	m_nCount = 0;
	memset( &m_ItemRoot, 0, sizeof(CTreeItem)-sizeof(_ITEMDATA));
	m_pCompareFunc = NULL;
	m_pDeleteFunc = NULL;
}
...
template<class _ITEMDATA>
POSITION CRGTreeT<_ITEMDATA>::FindAfterPos( const _ITEMDATA *pItemData, POSITION posParent/*=NULL*/)
{
	if( !m_pCompareFunc)
		return NULL;
	CTreeItem *pParent = posParent ? (CTreeItem*)posParent : m_ItemRoot.pTIChildFirst;

	if( (*m_pCompareFunc)( (LPARAM)pItemData, (LPARAM)&pParent->ItemData))
		return (POSITION)pParent;
	POSITION  pos;
	CTreeItem *pItem=NULL;
	if (pParent = pParent->pTIChildFirst)
	{
		if(pos = FindAfterPos(pItemData,(POSITION)pParent))
			return pos;
		if((pItem=pParent->pTINext) && pItem)
			return FindAfterPos(pItemData,(POSITION)pItem);
	}
	return NULL;
}
